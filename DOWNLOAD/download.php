...
if(isset($_POST['id']))
{
	...
	$url = 'https://sammanthan-jaffna.jkdesigns.app'.'/export/'.$id; //sivanarudselvan
	$cURL = curl_init();
	...
	$json = '[
  {
	...
	if(strpos($id, "Inventory")){
		foreach (json_decode($result1) as $set) {
			$arraydata[] = [$lan['SKU'] => $set->p1, $lan['NAMEEN'] => $set->p3, $lan['CATEGORY'] => $set->p2, $lan['BRAND'] => $set->p4, $lan['EXPIRY'] => $set->p5, $lan['PURCOST'] => $set->p6, $lan['PURCOSTPRO'] => $set->p7, $lan['PROFIT'] => $set->p11, $lan['UNIT'] => $set->p9, $lan['PRODUCTPRI'] => $set->p8, $lan['CUSTOCK'] => $set->p10, $lan['CUSTOVAL'] => $set->p17, $lan['RECORDENTER'] => $set->p18, $lan['INSTOCK'] => $set->p19, $lan['INITIALSTOVAL'] => $set->p20];
		}

	}elseif(strpos($id, "Summary")){
		$i = 0;
		...
		$objPHPExcel->getActiveSheet()->setCellValue('M3', 'Price Change');
		$objPHPExcel->getActiveSheet()->setCellValue('N3', 'Remark');
		$objPHPExcel->getActiveSheet()->setCellValue('O3', 'End Stock');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('K4', 'Credit');
		$objPHPExcel->getActiveSheet()->setCellValue('K5', 'Qty');
		$objPHPExcel->getActiveSheet()->setCellValue('L5', 'Amount');
		$objPHPExcel->getActiveSheet()->setCellValue('O4', 'Qty');
		$objPHPExcel->getActiveSheet()->setCellValue('P4', 'Amount');
		...
		foreach (json_decode($result1) as $set) {
			...
			$arraydata[] = [$lan['DATE'] => $set->z00, $lan['DSTART'] => $set->z01, $lan['SV'] => $set->z02, $lan['SI'] => $set->z03, $lan['SC'] => $set->z04, $lan['RV'] => $set->z05, $lan['RI'] => $set->z06, $lan['RC'] => $set->z07, $lan['GRADDED'] => $set->z08, $lan['DEND'] => $set->z09];
				...
				$objPHPExcel->getActiveSheet()->setCellValue('R4', 'Initial Cash');
				$objPHPExcel->getActiveSheet()->setCellValue('T4', $set->z00);
				$objPHPExcel->getActiveSheet()->setCellValue('R5', 'Day Cash');
				$objPHPExcel->getActiveSheet()->setCellValue('T5', $set->z01);
				$objPHPExcel->getActiveSheet()->setCellValue('R6', 'Total Cash');
				$INcash = $set->z00;
				$INcreadit = $set->z09;
				$discount = $set->z10;
				$INecash = $set->z03;
				...
				if($set->z01 == "Expense"){
					$ex++;
					$ex1 = $ex + 1;
					$objPHPExcel->getActiveSheet()->setCellValue('R'.$ex1, $set->z02);
					//$objPHPExcel->getActiveSheet()->setCellValue('S'.$ex1, $set->z02);
					$objPHPExcel->getActiveSheet()->setCellValue('T'.$ex1, $set->z03);
					$INex = $INex + $set->z03;
					$excount++;
				}else{
					if($lang == "en"){
					...
					$objPHPExcel->getActiveSheet()->setCellValue('K'.$excel, $set->z02c);
					$objPHPExcel->getActiveSheet()->setCellValue('L'.$excel, $set->z03c);
					$objPHPExcel->getActiveSheet()->setCellValue('O'.$excel, $set->z07);
					$objPHPExcel->getActiveSheet()->setCellValue('P'.$excel, $set->z08);
					...
					$objPHPExcel->getActiveSheet()->setCellValue('N'.$excel, $set->z10);
					$objPHPExcel->getActiveSheet()->setCellValue('M'.$excel, $set->z10t);
		...
		$objPHPExcel->getActiveSheet()->mergeCells('I3:L3'); //Sales
		$objPHPExcel->getActiveSheet()->mergeCells('I4:J4'); //Cash
		$objPHPExcel->getActiveSheet()->mergeCells('K4:L4'); //Credit
		$objPHPExcel->getActiveSheet()->mergeCells('O3:P3'); //End stock
		$excel++;
		//$objPHPExcel->getActiveSheet()->getStyle('P'.$excel)->getFont()->setBold(true);
		$objPHPExcel->getActiveSheet()->getStyle('P'.$excel)->getFont()->setSize(12);
		$excel2=$excel - ($excount + 1);
		$excel=$excel - $excount;
		$objPHPExcel->getActiveSheet()->setCellValue('P'.$excel++, '=SUM(P6:P'.$excel2.')');		
		...
		$objPHPExcel->getActiveSheet()->getStyle('L'.$excel)->getFont()->setSize(12);
		$objPHPExcel->getActiveSheet()->setCellValue('L'.$excel, '=SUM(L6:L'.$excel2.')');		
		...
		if($excount > 0){
			...
			$objPHPExcel->getActiveSheet()->getStyle('T6')->getFont()->setSize(12);
			$objPHPExcel->getActiveSheet()->getStyle('T6')->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->setCellValue('T6', '=SUM(T4:T5)');
			$objPHPExcel->getActiveSheet()->getStyle('T'.$add1)->getFont()->setSize(12);
			$objPHPExcel->getActiveSheet()->getStyle('T'.$add1)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->setCellValue('R'.$add1, 'Total Expense');
			$objPHPExcel->getActiveSheet()->setCellValue('T'.$add1, '=SUM(T8:T'.$add.')');
			$add2 = $add1 + 2;
			$objPHPExcel->getActiveSheet()->getStyle('T'.$add2)->getFont()->setSize(12);
			$objPHPExcel->getActiveSheet()->getStyle('T'.$add2)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->setCellValue('R'.$add2, 'End Cash');
			$objPHPExcel->getActiveSheet()->setCellValue('T'.$add2, '=T6-T'.$add1);
			$newstart = $add2 + 2;
			$newstart13 = $add2 + 14;
			$objPHPExcel->getActiveSheet()->getColumnDimension('T')->setWidth(13);
			$objPHPExcel->getActiveSheet()->getStyle('T4:T'.$newstart13)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_LKR_SIMPLE);			
		}
		
		$objPHPExcel->getActiveSheet()->getProtection()->setSheet(true);
		$objPHPExcel->getActiveSheet()->protectCells('A3:P'.$excel, 'PHPExcel');		
		...
		$objPHPExcel->getActiveSheet()->getStyle('L6:L'.$excel)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_LKR_SIMPLE);
		...
		$objPHPExcel->getActiveSheet()->getStyle('P6:P'.$excel)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_LKR_SIMPLE);
		
		//new summary
		$add3 = $add2 + 2;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'Initial Stock');
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, $INsctock);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'Initial Cash');
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, $INcash);
		$add3++;
		$add4 = $add3 - 3;
		$add5 = $add3 - 1;
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, '=SUM(T'.$add4.':T'.$add5.')');
		$objPHPExcel->getActiveSheet()->getStyle('T'.$add3)->getFont()->setSize(12);
		$objPHPExcel->getActiveSheet()->getStyle('T'.$add3)->getFont()->setBold(true);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'GR');
		$INgr = $INgr - $INpricepluse +  $INpriceminus;
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, $INgr);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'Price Change +');
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, $INpricepluse);
		$add3++;
		$add6 = $add3 - 3;
		$add7 = $add3 - 1;
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, '=SUM(T'.$add6.':T'.$add7.')');
		$objPHPExcel->getActiveSheet()->getStyle('T'.$add3)->getFont()->setSize(12);
		$objPHPExcel->getActiveSheet()->getStyle('T'.$add3)->getFont()->setBold(true);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'Expense');
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, "-".$INex);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'Credit');
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, "-".$INcreadit);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'Price Change -');
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, "-".$INpriceminus);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'Discount');
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, "-".$discount);
		$add3++;
		$add8 = $add3 - 5;
		$add9 = $add3 - 1;
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, '=SUM(T'.$add8.':T'.$add9.')');
		$objPHPExcel->getActiveSheet()->getStyle('T'.$add3)->getFont()->setSize(12);
		$objPHPExcel->getActiveSheet()->getStyle('T'.$add3)->getFont()->setBold(true);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'End Stock');
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, $INestock);
		$add3++;
		$objPHPExcel->getActiveSheet()->setCellValue('R'.$add3, 'End Cash');
		$add10 = $add3 - 2;
		$add11 = $add3 - 1;
		$objPHPExcel->getActiveSheet()->setCellValue('T'.$add3, '=T'.$add10.'-T'.$add11);
		...
		//New summary
		$objPHPExcel->getActiveSheet()->getStyle('R'.$newstart.':T'.$add3)->applyFromArray(
				array(
					'borders' => array(
						'top'     => array(
		...
		$newstart = $newstart + 2;
		$objPHPExcel->getActiveSheet()->getStyle('R'.$newstart.':T'.$newstart)->applyFromArray(
				array(
					'borders' => array(
						'bottom'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('R'.$newstartl.':R'.$newstart)->applyFromArray(
				array(
					'borders' => array(
						'left'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('T'.$newstartr.':T'.$newstart)->applyFromArray(
				array(
					'borders' => array(
						'right'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		//CASH AREA
		$objPHPExcel->getActiveSheet()->getStyle('R4:T'.$add2)->applyFromArray(
				array(
					'borders' => array(
						'top'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('R6:T6')->applyFromArray(
				array(
					'borders' => array(
						'bottom'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('R'.$add2.':T'.$add2)->applyFromArray(
				array(
					'borders' => array(
						'bottom'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('R'.$add1.':T'.$add1)->applyFromArray(
				array(
					'borders' => array(
						'bottom'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('R4:R'.$add2)->applyFromArray(
				array(
					'borders' => array(
						'left'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('A4:P'.$excel)->applyFromArray($styleThinBlackBorderOutline);
		...
		$objPHPExcel->getActiveSheet()->getStyle('A4:P5')->applyFromArray($styleThickBrownBorderOutline);
		// Set fills
		$objPHPExcel->getActiveSheet()->getStyle('A1:P1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
		$objPHPExcel->getActiveSheet()->getStyle('A1:P1')->getFill()->getStartColor()->setARGB('FFFFFF');
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$excel.':P'.$excel)->applyFromArray(
				array(
					'font'    => array(
						'bold'      => true
					),
		...
		$objPHPExcel->getActiveSheet()->getStyle('A4:P5')->applyFromArray(
				array(
					'font'    => array(
						'bold'      => true
					),
		...
		$objPHPExcel->getActiveSheet()->getStyle('L3:L'.$excel)->applyFromArray(
				array(
					'borders' => array(
						'right'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('I4:L4')->applyFromArray(
				array(
					'borders' => array(
						'bottom'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('M3:M'.$excel)->applyFromArray(
				array(
					'borders' => array(
						'right'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('K5:K'.$excel)->applyFromArray(
				array(
					'borders' => array(
						'right'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
		...
		$objPHPExcel->getActiveSheet()->getStyle('P4:P'.$excel)->applyFromArray(
				array(
					'borders' => array(
						'left'     => array(
							'style' => PHPExcel_Style_Border::BORDER_THIN
	...
	if($record['b4'] === $checkdate){
		
		if(strpos($record['b6'],"<br>") !== false){
			$str_br = explode ("<br>", $record['b6']);
			
				$objPHPExcel->getActiveSheet()->setCellValue('A'.$sales, $sales - 2);
				$objPHPExcel->getActiveSheet()->setCellValue('B'.$sales, $record['b1']);
		...
		foreach (json_decode($result1) as $set) {
			...
			$arraydata[] = [$lan['SERIALNO'] => $set->b1, $lan['CUSTOMER'] => $set->b2, $lan['DATE'] => $set->b4, $lan['TIME'] => $set->b10, $lan['SALESREP'] => $set->b5, $lan['NAMEEN'] => $b6s, $lan['TOTAL'] => $set->b7, $lan['CUSPAID'] => $set->b8, $lan['BALANCE'] => $set->b9, $lan['PAYMETHOD'] => $set->pay];
